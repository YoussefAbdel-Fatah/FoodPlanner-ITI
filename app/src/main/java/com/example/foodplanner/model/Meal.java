...
public class Meal {
    ...
    /**
     * Returns a list of non-empty Ingredient objects by pairing
     * each strIngredientN with its strMeasureN.
     */
    public List<Ingredient> getIngredientList() {
        ...
        for (int i = 0; i < ingredients.length; i++) {
            if (ingredients[i] != null && !ingredients[i].trim().isEmpty()) {
                ...
                list.add(new Ingredient(ingredients[i].trim(), measure));
            }
        }
        return list;
    }

    // Setters (for converting MealEntity → Meal)
